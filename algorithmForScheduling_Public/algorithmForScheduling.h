...
int fcfs_process(T* origin_queue, T* table_queue) {
	...
	for (int t = 0; t <= time; t++) {
		...
		for (int i = 0; i < tmp_size; i++) {
			...
			if (arr_time == t) {
				tmp_process = tmp.top();
				memory_queue.push(tmp_process);		// 메모리 큐는 넣으면 arrival time 기준으로 자동 정렬이 된다.
				tmp.pop();
			}
		}
...
// Strn 프로세스 함수 여기서는 큐의 포인터를 입력받고 그 큐에 Strn 프로세스 스케쥴링을 적용하여 대기열을 넣고 종료하는 함수
// 오리진 큐는 우선순위 큐가아닌 일반 적인 stl 큐를 사용
// 선점 형태이며 spn의 변형 방법이다. 프로세스의 실행 시간 기준으로 먼저 처리할 것을 결정한다.
template <typename T>
int srtn_process(T* origin_queue, T* table_queue) {
