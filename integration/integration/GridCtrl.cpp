...
void CGridCtrl::AutoSize(int yNum, UINT nAutoSizeStyle /*=GVS_DEFAULT*/)
{
    CDC* pDC = GetDC();
    if (!pDC)
        return;
    ...
    for (nCol = 0; nCol < nNumColumns; nCol++)
    {
        ...
        if( GetColumnWidth( nCol) > 0 )
        {
			...
			for (nRow = nStartRow; nRow <= nStartRow; nRow++)
			{
				m_arRowHeights[nRow] = 20;
			}
            for (nRow = nStartRow + 1; nRow <= nEndRow; nRow++)
            {
                if( GetRowHeight( nRow) > 0 )
                {
                    ...
                    if (size.cx >(int) m_arColWidths[nCol])
                        m_arColWidths[nCol] = 70;
                    if (size.cy >(int) m_arRowHeights[nRow])
                        m_arRowHeights[nRow] = totalYsize / yNum;
                }
            }
        }
    }
